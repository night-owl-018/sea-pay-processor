...
DEFAULT_RATE_FILE = os.path.join(CONFIG_DIR, "atgsd_n811.csv")
    ...
    for ship, dates in groups.items():
        ...
        for d in dates[1:]:
            if d == prev + timedelta(days=1):
                ...
                results.append({"ship":ship,"start":start.strftime("%m/%d/%Y"),"end":prev.strftime("%m/%d/%Y")})
                start = prev = d
...
def load_rates(rate_file, log):
    rates = {}
    if not os.path.exists(rate_file):
        log(f"[RATES] Missing CSV: {rate_file}")
        return rates
    ...
    log(f"[RATES] Loaded {len(rates)} records")
    return rates
...
def merge_with_bookmarks(log):
    pdfs = sorted([f for f in os.listdir(OUTPUT_DIR) if f.endswith(".pdf") and not f.startswith("MASTER")])
...
def run():
    ...
    rates = load_rates(DEFAULT_RATE_FILE, log)
    ...
    for file in os.listdir(DATA_DIR):
        ...
        rows = parse_rows(raw, extract_year_from_filename(path))
...
def upload_rate():
    f=request.files["file"]
    if f:
        f.save(os.path.join(CONFIG_DIR,f.filename))
    return redirect("/")
...
@app.route("/delete-template/<name>")
def delete_template(name):
    os.remove(os.path.join(TEMPLATE_DIR,name))
    return redirect("/")


@app.route("/delete-rate/<name>")
def delete_rate(name):
    os.remove(os.path.join(CONFIG_DIR,name))
    return redirect("/")


@app.route("/delete-data/<name>")
def delete_data(name):
    os.remove(os.path.join(DATA_DIR,name))
    return redirect("/")


@app.route("/delete-output/<name>")
